...
;(function ($) {
    ...
    // protected methods
    var initComponents = function() {
            $image = $th.find('.image-cropbox1');
            $frame = $th.find('.frame-cropbox1');
            $workarea = $th.find('.workarea-cropbox1');
            $membrane = $th.find('.membrane-cropbox1');
            $resize = $th.find('.resize-cropbox1');
        },
        initEvents = function() {
            // move frame
            $frame.on(EVENT_MOUSE_DOWN, frameMouseDown);
